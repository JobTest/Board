package com.pb.dashboard.external.monitor.sessions.table;
...
public class SessionsTablePanel extends VerticalLayout implements Property.ValueChangeListener {

    private static final long serialVersionUID = 2415499805703041712L;
    private Complex complex;
    private com.pb.dashboard.external.monitor.sessions.TableManager tableManager = new com.pb.dashboard.external.monitor.sessions.TableManager();
    ...
    public SessionsTablePanel(Complex complex, List<BpSession> sessions) {
    ...
    public SessionsTablePanel(Complex complex, List<BpSession> sessions, List<BpSession> sessionsWithoutDebt) {
        this.complex = complex;
        setMargin(true);
        addStyleName("table-panel");
        setSizeFull();
    ...
    private TableDataHolder loadTableDataHolder(BpSession session) {
        return ServiceFactory.getExternal().getSessionDetails(session.getId(), complex.getId());
    }
